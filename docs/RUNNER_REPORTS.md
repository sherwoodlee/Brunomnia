...
`--config <path>` loads an explicit bounded `.insorc`, `.insorc.json`, `.insorc.yaml`, `.insorc.yml`, or `package.json` `inso` property. Without it, the CLI searches upward from the explicit working directory or current directory. The supported pinned global options are `workingDir`, `ci`, `verbose`, and `printOptions`; CLI flags take precedence, `ci` supplies the first collection/suite when the identifier is omitted, and print diagnostics go to stderr so report stdout stays parseable. Unknown options are ignored. Up to 100 bounded string script definitions are retained but not executed yet; executable JavaScript/TypeScript config formats also remain disabled.
