...
The Results toolbar leaves every request attempt visible and filters only the assertion rows in the selected attempt, matching pinned Runner request cards. All, Passed, Failed, and Skipped use the retained assertion status. The name field uses pinned `fuzzysort` 1.9.0 behavior against the assertion name only, including its `-8000` garbage-match floor; request identity, URL, response status, and error text do not masquerade as assertion-name matches. Filters apply equally to active, latest, and reopened historical results without changing attempt rows, aggregate counters, saved reports, Console evidence, or exports. The direct response Tests pane uses the same controls and matcher.
