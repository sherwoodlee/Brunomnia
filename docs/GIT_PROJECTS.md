...
The commit-and-push preflight runs `git ls-remote --heads` through the same installed credential path. This proves that the named remote is reachable and its read credentials work without changing refs or files. It cannot prove write permission, branch-protection acceptance, fast-forward eligibility, or that the remote will remain available between preflight and push; those later failures preserve the new local commit for retry.
