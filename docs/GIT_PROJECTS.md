...
The `.git` directory is standard and remains usable from a terminal or another Git client. Provider-specific authentication, guided repository onboarding, credential validation, and automatic repository discovery are still tracked in [the parity ledger](PARITY.md). A pinned Milestone 221 source audit confirmed that standard Insomnia Git Sync does not expose force push, forced or remote branch deletion, rebase, cherry-pick, or arbitrary un-checked-out remote-history browsing as user-facing operations.
