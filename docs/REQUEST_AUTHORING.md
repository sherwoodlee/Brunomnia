...
By default the injected content-security policy blocks scripts, forms, automatic external subresources, and parent/top navigation privileges while allowing inline styles and embedded data images/fonts. The device-local **Preferences → HTML JavaScript** grant enables inline scripts and DOM interaction for trusted response bodies in both top-level and recursively selected multipart HTML; the viewer shows an active warning. Even when enabled, the response document's CSP plus the iframe sandbox omit same-origin, forms, popups, modals, downloads, parent/top navigation, external scripts, `eval`, and subresource network APIs. A followed page remains in that opaque sandbox but uses its own CSP; if the JavaScript grant is active, that destination can run scripts and network requests within the sandbox. Do not enable the grant for untrusted responses. Imports reset the grant off and folder/Git/encrypted revisions cannot turn it on.
...
**Source Code** shows the filtered/prettified textual representation with line numbers. **Raw Data** shows the charset-decoded inspection string without prettification, filters, or line numbers. Switching modes never changes the saved response; **Copy** follows the pinned upstream action and copies that inspection string regardless of the displayed mode. Exact entity bytes remain available to raw download and media viewers even when the inspection string differs. Available image/audio codecs and embedded PDF controls depend on the operating-system WebView. Filesystem-backed bodies, content sniffing beyond valid UTF-8 JSON and a leading HTML doctype, recursion outside the documented multipart safety bounds, automatic HTML subresource loading, and an upstream-equivalent response-link disable preference remain open.
