...
**Source Code** shows the filtered/prettified textual representation with line numbers. **Raw Data** shows the UTF-8-decoded inspection string without prettification, filters, or line numbers. Switching modes never changes the saved response; **Copy** follows the pinned upstream action and copies that inspection string regardless of the displayed mode. Exact entity bytes remain available to raw download and future media viewers even when the inspection string contains replacement characters. Image, PDF, audio, charset-aware text, filesystem-backed bodies, and interactive HTML preview parity remains open.
