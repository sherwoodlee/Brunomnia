...
**Source Code** shows the filtered/prettified textual representation with line numbers. **Raw Data** shows the exact stored UTF-8 string without prettification, filters, or line numbers. Switching modes never changes the saved response; **Copy** follows the pinned upstream action and copies the raw stored string regardless of the displayed mode. Image, PDF, audio, charset-aware binary, and interactive HTML preview parity remains open because Brunomnia currently stores decoded text instead of the upstream byte buffer.
