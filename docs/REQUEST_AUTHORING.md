...
Visual Preview renders detected or declared JSON through the filtered/prettified source path and detected or declared `text/html` in an opaque-origin sandboxed iframe. By default its injected content-security policy blocks scripts, forms, navigation privileges, and external network resources while allowing inline styles and embedded data images/fonts. The device-local **Preferences → HTML JavaScript** grant enables inline scripts and DOM interaction for trusted response bodies in both top-level and recursively selected multipart HTML; the viewer shows an active warning. Even when enabled, CSP plus the iframe sandbox omit same-origin, forms, popups, modals, downloads, parent/top navigation, external scripts, `eval`, and subresource network APIs. Same-frame link/script navigation remains operating-system WebView behavior, so do not enable the grant for untrusted responses. Imports reset the grant off and folder/Git/encrypted revisions cannot turn it on.
