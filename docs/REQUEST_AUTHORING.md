...
WebSocket upgrade while custom proxy/client identity or disabled-validation policy is active, upstream-style filesystem event/timeline storage, streaming plugin hooks, and live third-party compatibility fixtures remain open. Browser development uses deterministic local demo events and metadata; real transport runs in the Tauri app.
