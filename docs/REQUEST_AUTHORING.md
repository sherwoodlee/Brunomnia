...
The Preview toolbar stores one mode per request. **Visual Preview** renders JSON through the filtered/prettified source path and renders `text/html` in a sandboxed iframe whose injected content-security policy blocks scripts, forms, navigation privileges, and external network resources while allowing inline styles and embedded data images/fonts. **Source Code** shows the filtered/prettified textual representation with line numbers. **Raw Data** shows the exact stored UTF-8 string without prettification, filters, or line numbers. Switching modes never changes the saved response; **Copy** now follows the pinned upstream action and copies the raw stored string regardless of the displayed mode. Image, PDF, audio, CSV-table, multipart, charset-aware binary, and interactive HTML preview parity remains open because Brunomnia currently stores decoded text instead of the upstream byte buffer.
