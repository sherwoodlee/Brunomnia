...
The Preview toolbar stores one mode per request. **Visual Preview** renders JSON through the filtered/prettified source path, renders `text/html` in a sandboxed iframe whose injected content-security policy blocks scripts, forms, navigation privileges, and external network resources while allowing inline styles and embedded data images/fonts, and renders CSV media types as scrollable tables. It also routes declared `image/*`, `application/pdf`, and `audio/*` bodies into responsive image, embedded PDF, and native audio-control viewers backed by the exact decoded entity bytes. Media uses local revocable Blob URLs rather than data URLs or remote requests; switching responses or modes releases the URL. Empty bodies and WebView decode failures receive visible states, and raw downloads use useful binary/media extensions.
...
Visual Preview also parses `multipart/*` bodies using quoted or unquoted MIME boundaries and CRLF/LF framing. Choose among up to 100 parts by their disposition name/filename, inspect up to 100 unfolded headers, view JSON-prettified or plain decoded text, and save the complete selected part through a safe local text artifact. On-screen part content is capped at 1,000,000 decoded characters with a notice; Save part retains the complete stored text. Missing/incomplete boundaries and malformed header separators produce visible errors. This baseline does not reconstruct binary part bytes or recursively invoke HTML/CSV/media viewers.

**Source Code** shows the filtered/prettified textual representation with line numbers. **Raw Data** shows the UTF-8-decoded inspection string without prettification, filters, or line numbers. Switching modes never changes the saved response; **Copy** follows the pinned upstream action and copies that inspection string regardless of the displayed mode. Exact entity bytes remain available to raw download and media viewers even when the inspection string contains replacement characters. Available image/audio codecs and embedded PDF controls depend on the operating-system WebView. Charset-aware text, filesystem-backed bodies, byte-backed recursive multipart viewing, content sniffing, and interactive HTML preview parity remain open.
