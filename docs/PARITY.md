...
| Headless CLI and CI | Baseline | Bundled CLI lints/generates/exports OpenAPI and runs HTTP/GraphQL collections/tests with shared runner logic. The documented `dot`, `list`, `min`, `progress`, `spec`, and `tap` reporter names work alongside JSON/JUnit file output. Workspace JavaScript is disabled by default, with separate trusted-script, secondary-network, and file flags. Remaining Inso commands/flags, stronger portable script isolation, and signed containers remain. | [Inso CLI](https://developer.konghq.com/inso-cli/), [run test reference](https://developer.konghq.com/inso-cli/reference/run_test/) |
