...
| Preferences, shortcuts, themes, accessibility, and packaging | Baseline | Device-local system/dark/light themes, comfortable/compact density, editor font sizing, request/script defaults, preferred HTTP version, redirect/timeout/API-validation/auth-validation and system/manual proxy defaults, regular/bulk header and query editors, forced-vertical layout, editor wrapping/tabs/indent width/ligatures, separate interface/editor families and 8–24 px sizes, masked request/folder authentication and MCP/AI/Konnect credentials with global/per-field reveal, optional resolved-variable source/value disclosure, maximum redirects, timeline chunk size, response-history limit/environment filtering, response-viewer link disabling, off-by-default HTML-preview remote resources/JavaScript plus script network/file/vault grants, delete confirmation, and GraphQL auto-introspection exist. All 33 actions in the pinned `KeyboardShortcut` union accept up to eight unique combinations each, support add/remove/per-action/all reset, preserve explicitly cleared bindings, migrate legacy strings, warn on collisions, and dispatch from the window capture phase. Live targets cover collection/request settings, keyboard/general preferences, plugin descriptor reload, focused-editor autocomplete, send/options, environment editing/switching, HTTP method/URL/history/code generation, sidebar/create-menu/sidebar-toggle, response/cookies, request/folder creation, delete/duplicate/pin, variable disclosure, body beautification, GraphQL filtering, and tab lifecycle. Workspace v48 reproduces the upstream `Mod+N` create-menu versus `Mod+Alt+N` HTTP-request split without overwriting custom legacy bindings. A digest-pinned non-root CLI image and keyless signed multi-architecture GHCR release workflow add SBOM/provenance-backed headless packaging. A separate immutable-action workflow now builds unsigned macOS ARM64 DMG, Windows x64 NSIS/MSI, and Linux x64 AppImage/DEB/RPM installers, records only final platform-expected files in stable JSON and flat SHA-256 manifests, attests every installer digest with GitHub build provenance, retains main-branch artifacts for 30 days, and publishes tagged releases. The macOS app declares 10.15 because the pinned local llama.cpp runtime needs C++ filesystem support. Accessibility audit, updater, and desktop signing/notarization remain; unsigned bundles truthfully retain operating-system trust warnings. | [Keyboard shortcuts](https://developer.konghq.com/insomnia/keyboard-shortcuts/), [Insomnia documentation](https://developer.konghq.com/index/insomnia/) |
