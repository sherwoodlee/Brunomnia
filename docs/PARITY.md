...
| Request bodies | Baseline | JSON, text, URL-encoded, multipart text/files with duplicate fields and editable part filename/content type, binary payloads, and local JSON/conservative-XML beautification work; CLI supports the same finite modes. Generated snippets now retain exact multipart framing/file bytes and standalone binary bytes across all six local targets. Arbitrary authored per-part headers and some transport encoding compatibility remain. | [Requests](https://developer.konghq.com/insomnia/requests/) |
