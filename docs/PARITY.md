...
| Pre-request and after-response scripts | Baseline | Disposable desktop Workers cover scoped variables, parent folders, URL/header/body/auth/proxy/certificate request mutation, response helpers, console/tests, top-level await, selected safe modules, and separately opt-in mediated HTTP/vault access. Full bundled-library/Node coverage, advanced auth/body helpers, separately persisted base-environment mutation, async tests, secondary-cookie persistence, and broader Postman globals remain. CLI JavaScript requires an explicit trusted-workspace flag because Node `vm` is not a hostile-code boundary. | [Scripts](https://developer.konghq.com/insomnia/scripts/) |
...
| Import and export formats | Baseline | Existing formats/scopes map advanced Insomnia/Postman auth, custom HTTP methods, explicit path/query/header rows with descriptions and multiline values, OpenAPI path parameters, cookie jars, environment ancestry, and nested Insomnia v4/v5 folders with headers/auth/variables/scripts/docs. Partial scripts in other formats, external files, WSDL placeholders, Socket.IO/MCP downgrades, and omitted binary bytes remain explicit bounds. | [Import/export reference](https://developer.konghq.com/insomnia/import-export/) |
