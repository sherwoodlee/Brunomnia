...
| Mock servers | Baseline | Real native loopback mock server with routes, parameters, headers, delays, CORS, request-aware output from headers/query/path segments/raw or parsed JSON/form bodies with the `default` filter, bounded `assign`/`if`/`unless`/`else`/`raw` controls, three legacy dynamic tokens, direct active-response-to-new-route creation or selected-route response overwrite, and optional AI generation from pasted prompt/spec/example material, an explicitly fetched/reviewed specification URL, or credential-redacted active-request/latest-response context. Multipart field parsing, Faker breadth, broader Liquid operators/filters, response-pane server selection, and hosted/self-host deployment workflows remain. | [Mock servers](https://developer.konghq.com/insomnia/mock-servers/) |
