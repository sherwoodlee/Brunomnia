...
| Service integrations | Baseline | A pull-only Konnect adapter lists control planes and maps Gateway Services plus every supported HTTP/HTTPS method/path/protocol combination, WS/WSS path, and gRPC/GRPCS service-method path into native requests. Regex path parameters, Host/header constraints, gRPC metadata, protocol-specific reviewed proxy variables, remote-template sanitization, stable managed keys, stale-managed-header removal, and preservation of local query/auth/body/transport/script/test/custom-header work are covered. Missing-service, expression-router, SNI, and L4 routes remain visible with reasons; pinned Insomnia likewise skips SNI/L4 and never pushes Gateway configuration, so those prior ledger requirements were false. Automatic all-control-plane project/workspace reconciliation, control-plane proxy-URL defaults, route-folder hierarchy, reviewed expression conversion, live-tenant fixtures, and newly documented adapters remain. | [Konnect integration](https://developer.konghq.com/insomnia/konnect-integration/) |
