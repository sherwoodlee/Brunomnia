...
| gRPC | Baseline | Authored `grpc:`, `grpcs:`, `http:`, and `https:` endpoints, reflection, bounded multi-file/folder proto-tree import with active/entry selection and cross-file compilation, legacy single-source migration, dynamic JSON/protobuf mapping, all four call shapes, inherited/overridden secure-endpoint certificate validation, workspace CA trust, and host/port-scoped PEM client identity work. Custom proxy transport, PFX/PKCS#12 identity, and additional metadata/schema workflows remain. | [Insomnia documentation](https://developer.konghq.com/index/insomnia/) |
