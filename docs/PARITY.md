...
| gRPC | Baseline | Authored `grpc:`, `grpcs:`, `http:`, and `https:` endpoints, reflection, pasted proto compilation, dynamic JSON/protobuf mapping, all four call shapes, inherited/overridden secure-endpoint certificate validation, and domain-scoped PEM client identity work. Importable proto trees, custom proxy transport, custom CA/PFX identity, and additional metadata/schema workflows remain. | [Insomnia documentation](https://developer.konghq.com/index/insomnia/) |
