...
| Mock servers | Baseline | Real native loopback mock server with routes, parameters, headers, delays, CORS, three dynamic tokens, direct active-response-to-new-route creation, and optional AI generation from pasted prompt/spec/example material or explicitly selected credential-redacted active-request/latest-response context. Liquid/Faker breadth, response-to-existing-route overwrite, binary mock bodies, URL-to-mock generation, and hosted/self-host deployment workflows remain. | [Mock servers](https://developer.konghq.com/insomnia/mock-servers/) |
