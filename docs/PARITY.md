...
| Git Sync and version control | Baseline | Split-YAML projects in standard repositories now support init/clone/status, selected/all stage and unstage, selected/all unstaged discard with index preservation, aggregate and confined per-file working/staged diffs including bounded untracked text, commit or credential-preflighted commit-and-push with explicit later partial-failure reporting, a bounded current-branch history with author/date/parents/decorations and selected-commit patches, local branch create/switch/merged-delete, explicit fetch/prune, remote-only branch discovery and tracking checkout, remotes, pull/push, clean-tree-guarded merge, three-way text resolution, binary side selection, and abort. Provider-specific authentication/onboarding, un-checked-out remote history, force/local-remote deletion, rebase/cherry-pick, automatic discovery, and broader edge-case fixtures remain. | [Storage and Git Sync](https://developer.konghq.com/insomnia/storage/), [Git Sync](https://developer.konghq.com/insomnia/git-sync/) |
