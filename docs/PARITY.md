...
| Pre-request and after-response scripts | Baseline | Disposable desktop Workers cover distinct global/collection base and selected stores, documented seven-level aliases/priority, ID/name parent folders, query-string/object URL mutation, keyed-array Basic/Bearer/API-key updates, proxy/certificate helpers, response facades, ordered async Chai-style tests, bounded adapters for all currently documented external-library and Node-module names, and separately opt-in mediated HTTP/vault access with cookie/response continuity. Full npm-package/Chai/Lodash behavior, file-backed helpers, external-vault scripts, and deprecated Postman interfaces remain. CLI JavaScript requires an explicit trusted-workspace flag because Node `vm` is not a hostile-code boundary. | [Scripts](https://developer.konghq.com/insomnia/scripts/), [environments](https://developer.konghq.com/insomnia/environments/) |
