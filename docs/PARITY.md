...
| Cookies, chaining, and dynamic variables | Complete | Persistent editable jar, automatic primary/secondary-request Set-Cookie storage, send/store policy, active-environment-aware response chaining, seven-level base/selected global/collection/folder/iteration/local precedence with disabled-row masking, exact pinned Faker 9.7 coverage for all 118 names, JSONPath Plus 10.4 safe predicates/unions/slices/recursive selectors, Base64/UUID/time/hash/cookie/request/prompt tags, approved four-provider external-vault tags, and desktop-only 5 MB canonical-root-confined File tags share one renderer. The account-free Tags dialog builds compatible syntax and inserts or replaces it across every request field that renderer supports. | [Template tags](https://developer.konghq.com/insomnia/template-tags/), [dynamic variables](https://developer.konghq.com/insomnia/dynamic-variables/), [request chaining](https://developer.konghq.com/how-to/chain-requests/) |
...
- Account-free thirteen-family builder with escaped syntax and append/replace destinations for every shared-renderer request field: [`src/components/TemplateTagDialog.tsx`](../src/components/TemplateTagDialog.tsx), [`src/lib/templateTagBuilder.ts`](../src/lib/templateTagBuilder.ts), and [`src/App.tsx`](../src/App.tsx)
