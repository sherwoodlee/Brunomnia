...
| REST/HTTP execution | Baseline | Native execution, custom token-valid methods, encoded `{path}` substitution, repeated query keys, response inspection, redirects, timeouts, TLS validation, proxy exclusions, domain-scoped PEM identity, per-request cookie policy, and device-local Default/HTTP 1.0/HTTP 1.1/HTTP/2/HTTP/2 Prior Knowledge preference work. Negotiated native versions appear in response evidence. Compression/client-network details, HTTP/3, and browser forbidden-header limits remain. | [Requests](https://developer.konghq.com/insomnia/requests/) |
