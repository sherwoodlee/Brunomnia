...
Last reconciled: **2026-07-21** against the current Kong Developer documentation for Insomnia. Milestone-specific source checks after that date are recorded in their verification files.
...
| Plugins and extension API | Complete | Disabled-by-default local CommonJS plugins run in disposable Workers with explicit grants. Desktop HTTP, GraphQL, gRPC, and collection sends expose the documented request/body/header/parameter/auth/environment/cookie/settings context, exact response bytes/stream/duplicate headers, local store, normalized secondary requests, app dialog/prompt/clipboard/path helpers, and host-mediated data import/export with a separate private-value grant. Request, request-group, workspace, and document actions receive selected/descendant models; enabled action-granted request exports appear on matching ordinary and pinned request menus, request-group exports appear on folder menus, and document exports appear on API Design project cards. Workspace exports remain workbench-callable because the pinned renderer bridge has no mounted workspace-action surface. Themes support mapped style colors plus bounded sanitized raw CSS. Linked local package metadata and explicit reload preserve the package root while disabling changed code and clearing grants. Bounded manual discovery scans direct roots, `node_modules`, and scoped folders; up to 500 reviewable package-local JavaScript/CJS/JSON modules resolve relative CommonJS paths, and unchanged rediscovery preserves reviewed authority. Strict unscoped `insomnia-plugin-*` names can also be fetched from npm or a custom registry through same-origin metadata redirects and confined HTTPS/exact-custom-origin tarballs with proxy/no-proxy, validation/CA, per-package SHA-1, tar-header/path/size/count/depth, manifest, version, and entry checks; package scripts never run and the fetched package enters the same disabled zero-grant review flow. Local and registry packages retain bounded reviewable pure-CommonJS production graphs: canonical local `node_modules` or recursively fetched exact/tag/semver dependencies are confined to 50 packages, 2,000 files, 20 MB of source, one compatible version per name, and explicit per-package grants. Optional failures warn; peer, alias, Git/HTTP/file/workspace, ESM-only, binary, native-addon, lifecycle-script, cross-package-relative, and conflicting-version behavior stays denied. Registry identity and dependency source survive migration/import, explicit current-registry refetch, and same-package upsert: byte-identical code/maps/entries/requests preserve stable identity and reviewed authority while changed packages disable and clear grants/data/theme. Pinned settings likewise reinstall by package name and explicitly reload; no automatic update checker or file watcher exists. Baseline `buffer`/`path`/`crypto`, explicitly granted `events`, and exact vendored `uuid` 11.1.1 plus `ajv` 8.18.0 match the pinned curated registry, manifest requests, denial messages, and crypto/UUID/AJV regression surface in both desktop and CLI workers. `--allow-plugins` additionally runs granted request/response hooks and tags with the same reviewed dependency graph for portable collection/suite HTTP/GraphQL sends inside stricter resource-limited workers without host RPC or persistent writes. The pinned desktop loader discovers and executes plugin entrypoints through CommonJS `require`; lifecycle scripts, native addons, ambient Node/process authority, ESM/export-map graphs, and conflicting dependency versions are incidental or unsupported loader behavior rather than documented extension contracts. The pinned sandbox-demo contract is covered by frozen process metadata, `INSOMNIA_TEMPLATE_SANDBOX`, safe `globalThis`, `context.util.nodeOS()`, `Buffer`, URL, Web Crypto, and relative CommonJS modules. Pinned Inso has no arbitrary desktop plugin host, so CLI host RPC and user-invoked actions are not cross-surface requirements. Pinned production settings expose only a registry URL; development-only `NODE_AUTH_TOKEN` injection is not a production registry-authentication feature. Pinned realtime connect routes bypass plugin hooks, so streaming hooks are not a parity requirement. | [Plugins](https://developer.konghq.com/insomnia/plugins/), [plugin reference](https://developer.konghq.com/insomnia/plugins/plugin-reference/), [hooks and actions](https://developer.konghq.com/insomnia/plugins/hooks-and-actions/) |
| Secrets and external vaults | Complete | A passphrase-derived AES-256-GCM local vault keeps decrypted values in memory, resolves `vault.*` variables, and reaches desktop scripts only through a device-local off-by-default grant. Private global environments and private collection sub-environments expose pinned String/JSON/Secret rows only with unlocked vault authority: values stay owner-bound and empty in workspace JSON, inputs are masked/revealable, direct entries win collisions, conversion to plaintext requires confirmation, raw/shared/base/folder scopes fail closed, and delete/duplicate/reset clean matching encrypted entries. Collection aliases are derived from each request's owning selected private sub-environment across direct/dependent HTTP, OAuth/schema, runner, scripts, plugins, code generation, GraphQL, gRPC, realtime, and unit tests without cross-collection leakage. Workspace v49 strips imported Secret authority, omits private collection environments from default export/Git/project/encrypted-sync payloads, preserves matching local private environments on pulls, and omits every Secret row even from explicit private-value exports; non-portable Insomnia ciphertext is warned and discarded. Optional saved vault keys, external-provider profiles, request/folder/MCP OAuth runtime data, and reusable Git credentials use macOS Keychain, Windows Credential Manager, or Linux Secret Service through a generation-switched 1,200-byte chunk manifest with exact length and SHA-256 integrity; legacy macOS items and envelopes migrate without returning stored keys to the renderer. AWS, GCP, Azure, and HashiCorp adapters support ambient official-CLI chains plus bounded non-syncable protected profiles, credential-bound approvals/cache identity, HCP direct HTTPS, guided official Azure CLI browser/device-code authentication, and explicit expiry renewal without an account, entitlement, borrowed client ID, or secret child arguments. Portable CLI external-vault authority remains invocation-only and rejects device-profile arguments. Pinned source exposes explicit Azure renewal and only `insomnia.vault` to scripts, so silent renewal and external-provider scripting APIs are not user-facing requirements. | [External vault integration](https://developer.konghq.com/insomnia/external-vault/), [environments](https://developer.konghq.com/insomnia/environments/) |
| Cloud sync and collaboration | Complete | Account-free encrypted collaboration supports user-selected self-hosted filesystem/WebDAV files, passphrase compatibility, and per-device X25519 identities whose public invite codes wrap a fresh AES-256-GCM content key independently for up to 100 recipients. Private keys remain in macOS Keychain, Windows Credential Manager, or Linux Secret Service; recipient fingerprints and the complete roster are authenticated, removal rotates the next revision key, and recipient-encrypted files cannot silently downgrade. Filtered workspace data plus bounded object-scoped Collection, Environment, API Design, Mock Server, and MCP Client commits cross the boundary; private environments, credentials, response/runtime state, paths, plugins, and preferences stay local. Staging, dirty detection, 200-commit history, restore-as-commit, 50 branches per resource, checkout/delete, three-way JSON merge, explicit mine/theirs/manual conflict resolution, optimistic remote revision rejection, force push, and five-second clean auto-pull are available without an account, subscription, or hosted dependency. Pinned source exposes encrypted commit/push, resource branches/history/rollback/merge, and synchronization, but no product comments or presence surface; a reviewed shared-file path is Brunomnia's free discovery adapter. | [Storage options](https://developer.konghq.com/insomnia/storage/), [data security](https://developer.konghq.com/insomnia/end-to-end-encryption/) |
...
| SSO, RBAC, SCIM, audit, and organization controls | Complete | Workspace v51 provides free organization identity, exact pinned permission identifiers, owner/admin/editor/viewer members, manual/SCIM provenance, teams, 30-day invite/reinvite/revoke/accept flows, ownership transfer guards, verified HTTPS domains, exact Cloud Sync/Local Vault/Git Sync rules, and bounded governance/SCIM logs. Native OIDC uses HTTPS discovery, authorization code plus PKCE, random state/nonce, ID-token issuer/audience/signature validation, and optional access-token hash validation with OS-protected confidential-client secrets. Native SAML 2.0 pins the IdP certificate and validates configured response/assertion signatures, issuer, status, audience, destination/recipient, bearer confirmation, `InResponseTo`, RelayState, time windows, and replay. Enabling SSO disables local actor switching; authenticated email must match an active member on a verified domain. The self-hosted bearer SCIM 2.0 connector implements discovery, schemas, Users, Groups, filter/pagination, create/read/replace/patch/deactivate/delete, email matching, non-destructive manual users/teams, group membership, last-owner refusal, one-time OS-protected token verifiers, expiry/automatic renewal, and request logs for Okta/Azure behind an operator-owned HTTPS reverse proxy. Direct/team grants scope request collections and API designs before sidebar/dashboard/tab/pin/Runner activation. Pinned docs expose SCIM request logs, not a separate general remote audit export. No account, plan, seat, subscription, or entitlement service is required. | [SSO](https://developer.konghq.com/insomnia/sso/), [SCIM](https://developer.konghq.com/insomnia/scim/), [RBAC](https://developer.konghq.com/insomnia/rbac/), [Organizations](https://developer.konghq.com/insomnia/organizations/) |
| Preferences, shortcuts, themes, accessibility, and packaging | Complete | Device-local system/dark/light themes, density, editor/layout/font/request/script/transport/proxy/history/preview/security preferences, masked credentials with explicit reveal, and all 33 pinned keyboard actions with bounded multi-bindings, collision warnings, clearing, reset, migration, and live capture-phase targets are account-free. The shell now provides skip navigation, a labeled main landmark, real tab/tabpanel relationships with Arrow/Home/End activation, polite live status, visible focus, reduced-motion handling, increased-contrast rules, forced-colors support, and audited accent/secondary-text contrast. Workspace v52 stores stable/beta signed-update preferences; packaged builds check at startup and every three hours, support manual checks, verify Tauri signatures before staging, restart to apply on macOS/Windows, report Linux package releases, and honor development/administrator/portable guards. Main CI keeps truthful unsigned ARM64 macOS, Windows x64, and Linux x64 installers with checksums/provenance; credential-required tags build universal notarized macOS, Authenticode-signed Windows, Linux packages, signed updater archives, and separate rolling stable/beta manifests without a commercial service. Exact local and rendered evidence is recorded in [Milestone 280 verification](QA_MILESTONE_280.md). | [Keyboard shortcuts](https://developer.konghq.com/insomnia/keyboard-shortcuts/), [Insomnia documentation](https://developer.konghq.com/index/insomnia/) |
...
- Cross-platform credential-store master-key retrieval, bounded AES-256-GCM credential envelope, workspace-identity authentication, legacy macOS migration, strict parsing, and tamper rejection: [`src-tauri/src/platform_keyring.rs`](../src-tauri/src/platform_keyring.rs) and [`src-tauri/src/runtime_credentials.rs`](../src-tauri/src/runtime_credentials.rs)
...
## Milestone 258 acceptance evidence

- Pinned scripting surface, read-only vault, encrypted PEM, and package-internal boundary audit: [`docs/QA_MILESTONE_258.md`](QA_MILESTONE_258.md)
- Disposable resource-limited CLI Workers, restricted VM execution, host-mediated secondary requests/files, and focused isolation coverage: [`cli/scriptRuntime.ts`](../cli/scriptRuntime.ts) and [`cli/scriptRuntime.test.ts`](../cli/scriptRuntime.test.ts)
- Shared pinned vault surface plus encrypted PKCS#8/legacy PEM identity handling across native consumers: [`src/lib/scriptSandbox.ts`](../src/lib/scriptSandbox.ts), [`src-tauri/src/client_identity.rs`](../src-tauri/src/client_identity.rs), and [`docs/QA_MILESTONE_258.md`](QA_MILESTONE_258.md)
- Full local/release gates, signed multi-architecture image publication, and independent exact-identity Cosign verification: [`docs/QA_MILESTONE_258.md`](QA_MILESTONE_258.md)

## Milestone 259 acceptance evidence

- Pinned Runner upload, result-byte, live/saved card, complete UI/CLI, and transport-boundary audit: [`docs/QA_MILESTONE_259.md`](QA_MILESTONE_259.md)
- Exact pinned `chardet` 2.1.1 auto-detection with real Shift_JIS regression: [`package.json`](../package.json), [`src/lib/runner.ts`](../src/lib/runner.ts), and [`src/lib/runner.test.ts`](../src/lib/runner.test.ts)
- Compressed transfer-byte Runner snapshots/live cards with decoded browser fallback and focused regression: [`src/lib/runner.ts`](../src/lib/runner.ts) and [`src/lib/runner.test.ts`](../src/lib/runner.test.ts)
- Packaged Runner/Inso command smoke, native/frontend/build gates, and exact verification record: [`scripts/cli-runner-preview-smoke.mjs`](../scripts/cli-runner-preview-smoke.mjs) and [`docs/QA_MILESTONE_259.md`](QA_MILESTONE_259.md)

## Milestone 262 acceptance evidence

- Pinned plugin reference and operation audit: [`docs/QA_MILESTONE_262.md`](QA_MILESTONE_262.md)
- Documented request/response/app/data/action/theme context and permission boundary: [`src/lib/plugins.ts`](../src/lib/plugins.ts), [`src/components/PluginWorkbench.tsx`](../src/components/PluginWorkbench.tsx), and [`src/types.ts`](../src/types.ts)
- URL-encoding persistence and execution/interchange continuity: [`src/lib/request.ts`](../src/lib/request.ts), [`src/lib/http.ts`](../src/lib/http.ts), [`src/lib/storage.ts`](../src/lib/storage.ts), [`src/lib/interchange/insomnia.ts`](../src/lib/interchange/insomnia.ts), and [`src/lib/interchange/exporters.ts`](../src/lib/interchange/exporters.ts)
- Linked local package metadata/reload and desktop-path command: [`src-tauri/src/plugin.rs`](../src-tauri/src/plugin.rs), [`src-tauri/src/lib.rs`](../src-tauri/src/lib.rs), and [`src/components/PluginWorkbench.tsx`](../src/components/PluginWorkbench.tsx)
- Opt-in CLI request/response hook and template runtime: [`cli/pluginRuntime.ts`](../cli/pluginRuntime.ts) and [`cli/brunomnia.ts`](../cli/brunomnia.ts)
- Context, action, isolation, persistence, interchange, execution, and CLI regressions: [`src/lib/plugins.test.ts`](../src/lib/plugins.test.ts), [`cli/pluginRuntime.test.ts`](../cli/pluginRuntime.test.ts), [`src/lib/request.test.ts`](../src/lib/request.test.ts), [`src/lib/storage.test.ts`](../src/lib/storage.test.ts), and [`src/lib/interchange/exporters.test.ts`](../src/lib/interchange/exporters.test.ts)

## Milestone 263 acceptance evidence

- Pinned install/path/module-map audit and exact local/release gates: [`docs/QA_MILESTONE_263.md`](QA_MILESTONE_263.md)
- Canonical package walking, direct/scoped discovery, bounds, symlink/dependency exclusion, package metadata, and native regressions: [`src-tauri/src/plugin.rs`](../src-tauri/src/plugin.rs) and [`src-tauri/src/lib.rs`](../src-tauri/src/lib.rs)
- Isolated multi-file CommonJS resolution, operation-local caching, traversal/bare-module denial, and full-package permission inference: [`src/lib/plugins.ts`](../src/lib/plugins.ts), [`src/lib/plugins.test.ts`](../src/lib/plugins.test.ts), [`cli/pluginRuntime.ts`](../cli/pluginRuntime.ts), and [`cli/pluginRuntime.test.ts`](../cli/pluginRuntime.test.ts)
- Package discovery/review, changed-code authority removal, and unchanged rediscovery continuity: [`src/components/PluginWorkbench.tsx`](../src/components/PluginWorkbench.tsx) and [`src/styles.css`](../src/styles.css)
- Bounded module-map migration and native IPC typing: [`src/types.ts`](../src/types.ts), [`src/lib/storage.ts`](../src/lib/storage.ts), [`src/lib/storage.test.ts`](../src/lib/storage.test.ts), and [`src/lib/project.ts`](../src/lib/project.ts)

## Milestone 264 acceptance evidence

- Pinned registry, crypto, manifest, UUID, AJV, and grant-lifecycle audit plus exact local/release gates: [`docs/QA_MILESTONE_264.md`](QA_MILESTONE_264.md)
- Baseline/grantable registry, exact denials, deterministic vendored factories, crypto compatibility, request inference, and authority identity helpers: [`src/lib/pluginModules.ts`](../src/lib/pluginModules.ts), [`src/lib/pluginVendored.generated.ts`](../src/lib/pluginVendored.generated.ts), and [`src/lib/pluginModules.test.ts`](../src/lib/pluginModules.test.ts)
- Isolated exact-version generation inputs and reproducible bundle generator: [`scripts/plugin-vendored/package.json`](../scripts/plugin-vendored/package.json), [`scripts/plugin-vendored/package-lock.json`](../scripts/plugin-vendored/package-lock.json), and [`scripts/generate-plugin-vendored.mjs`](../scripts/generate-plugin-vendored.mjs)
- Native manifest parsing plus install/discovery/reload/edit/import grant lifecycle: [`src-tauri/src/plugin.rs`](../src-tauri/src/plugin.rs), [`src/components/PluginWorkbench.tsx`](../src/components/PluginWorkbench.tsx), [`src/lib/storage.ts`](../src/lib/storage.ts), and [`src/lib/interchange/importers.test.ts`](../src/lib/interchange/importers.test.ts)
- Shared desktop/CLI module execution and pinned crypto/UUID/AJV/runtime regressions: [`src/lib/plugins.ts`](../src/lib/plugins.ts), [`cli/pluginRuntime.ts`](../cli/pluginRuntime.ts), and [`cli/pluginRuntime.test.ts`](../cli/pluginRuntime.test.ts)

## Milestone 265 acceptance evidence

- Pinned installer/name/registry/tarball/dependency audit plus exact local/release gates: [`docs/QA_MILESTONE_265.md`](QA_MILESTONE_265.md)
- Bounded metadata/tarball networking, proxy/TLS/CA policy, SHA-1 verification, in-memory gzip/tar confinement, identity checks, and native regressions: [`src-tauri/src/plugin.rs`](../src-tauri/src/plugin.rs) and [`src-tauri/src/lib.rs`](../src-tauri/src/lib.rs)
- Strict client-side name validation, device-local registry preference migration, typed IPC, and focused regressions: [`src/lib/plugins.ts`](../src/lib/plugins.ts), [`src/lib/preferences.ts`](../src/lib/preferences.ts), [`src/lib/storage.ts`](../src/lib/storage.ts), [`src/lib/project.ts`](../src/lib/project.ts), [`src/lib/plugins.test.ts`](../src/lib/plugins.test.ts), and [`src/lib/storage.test.ts`](../src/lib/storage.test.ts)
- Fetch-for-review workbench flow, explicit disabled/zero-grant installation boundary, and responsive registry controls: [`src/components/PluginWorkbench.tsx`](../src/components/PluginWorkbench.tsx) and [`src/styles.css`](../src/styles.css)

## Milestone 266 acceptance evidence

- Pinned settings reinstall, name-keyed discovery, explicit reload, dependency, and watcher audit plus exact local/release gates: [`docs/QA_MILESTONE_266.md`](QA_MILESTONE_266.md)
- Strict shared registry identity validation, unchanged/changed upsert planning, stable authority/data/theme lifecycle, and focused regressions: [`src/lib/pluginModules.ts`](../src/lib/pluginModules.ts) and [`src/lib/pluginModules.test.ts`](../src/lib/pluginModules.test.ts)
- Bounded identity migration/import with invalid-value removal and safe imported-authority reset: [`src/types.ts`](../src/types.ts), [`src/lib/storage.ts`](../src/lib/storage.ts), and [`src/lib/storage.test.ts`](../src/lib/storage.test.ts)
- Explicit installed-plugin registry check, review-form update targeting, same-package replacement, source detachment, and local/manual reset repair: [`src/components/PluginWorkbench.tsx`](../src/components/PluginWorkbench.tsx)

## Milestone 267 acceptance evidence

- Pinned Yarn production-graph/copy/native-require versus template-sandbox exclusion audit plus exact local/release gates: [`docs/QA_MILESTONE_267.md`](QA_MILESTONE_267.md)
- Bounded local and registry dependency graph capture, npm range selection, per-package metadata/tarball/checksum/identity validation, and native regressions: [`src-tauri/src/plugin.rs`](../src-tauri/src/plugin.rs)
- Persisted dependency map/package migration, authority-sensitive identity, strict package-root extraction, and safe import behavior: [`src/types.ts`](../src/types.ts), [`src/lib/storage.ts`](../src/lib/storage.ts), [`src/lib/pluginModules.ts`](../src/lib/pluginModules.ts), [`src/lib/storage.test.ts`](../src/lib/storage.test.ts), and [`src/lib/pluginModules.test.ts`](../src/lib/pluginModules.test.ts)
- Explicit dependency grants, cross-package traversal denial, shared desktop/CLI CommonJS execution, complete review UI, and runtime regressions: [`src/lib/plugins.ts`](../src/lib/plugins.ts), [`cli/pluginRuntime.ts`](../cli/pluginRuntime.ts), [`cli/pluginRuntime.test.ts`](../cli/pluginRuntime.test.ts), and [`src/components/PluginWorkbench.tsx`](../src/components/PluginWorkbench.tsx)

## Milestone 268 acceptance evidence

- Pinned request/folder/design placement, workspace-action renderer absence, and registry-setting audit plus exact local/release gates: [`docs/QA_MILESTONE_268.md`](QA_MILESTONE_268.md)
- Enabled/granted contextual catalog, exact target rebinding, authority identity, stale-result refusal, and request/store persistence: [`src/lib/plugins.ts`](../src/lib/plugins.ts) and [`src/lib/plugins.test.ts`](../src/lib/plugins.test.ts)
- Ordinary/pinned request, folder, and API Design fixed-menu placement with bounded feedback: [`src/App.tsx`](../src/App.tsx), [`src/styles.css`](../src/styles.css), [`src/components/Icon.tsx`](../src/components/Icon.tsx), and [`src/App.pluginActions.test.tsx`](../src/App.pluginActions.test.tsx)

## Milestone 269 acceptance evidence

- Pinned local vault-key storage, automatic retrieval, and exact settings-label audit: [`docs/QA_MILESTONE_269.md`](QA_MILESTONE_269.md)
- Project-scoped macOS Keychain, Windows Credential Manager, and Linux Secret Service status/retain/forget plus saved-key unlock/save operations and stale-key refusal: [`src-tauri/src/platform_keyring.rs`](../src-tauri/src/platform_keyring.rs), [`src-tauri/src/vault_keychain.rs`](../src-tauri/src/vault_keychain.rs), and [`src-tauri/src/lib.rs`](../src-tauri/src/lib.rs)
- Startup/project-switch auto-unlock and the account-free **Save encrypted vault key locally** control: [`src/App.tsx`](../src/App.tsx), [`src/components/SecurityWorkbench.tsx`](../src/components/SecurityWorkbench.tsx), and [`src/lib/security.ts`](../src/lib/security.ts)
- Reset, soft-delete/restore, permanent-purge, Empty, renderer-authority, and static-control regressions: [`src-tauri/src/workspace_store.rs`](../src-tauri/src/workspace_store.rs), [`src/lib/security.native.test.ts`](../src/lib/security.native.test.ts), and [`src/components/SecurityWorkbench.vaultKey.test.tsx`](../src/components/SecurityWorkbench.vaultKey.test.tsx)

## Milestone 277 acceptance evidence

- Pinned Secret-scope, Azure-renewal, scripting-vault, and safe-storage reconciliation plus complete local/remote verification: [`docs/QA_MILESTONE_277.md`](QA_MILESTONE_277.md)
- Workspace v49 private collection Secret normalization, owner-bound runtime scoping, reset/import stripping, and sync/project/export boundaries: [`src/types.ts`](../src/types.ts), [`src/lib/environmentSecrets.ts`](../src/lib/environmentSecrets.ts), [`src/lib/storage.ts`](../src/lib/storage.ts), [`src/lib/security.ts`](../src/lib/security.ts), [`src/lib/project.ts`](../src/lib/project.ts), and [`src/lib/interchange/exporters.ts`](../src/lib/interchange/exporters.ts)
- Private collection editor controls, masked values, conversion/privacy/delete guards, and scoped direct/dependent/runner/script/plugin/codegen/schema/realtime/unit-test contexts: [`src/App.tsx`](../src/App.tsx), [`src/components/AutomationWorkbench.tsx`](../src/components/AutomationWorkbench.tsx), and [`src/components/UnitTestWorkbench.tsx`](../src/components/UnitTestWorkbench.tsx)
- Generation-switched chunk manifests with exact length and SHA-256 integrity across macOS Keychain, Windows Credential Manager, and Linux Secret Service, including legacy macOS migration: [`src-tauri/src/platform_keyring.rs`](../src-tauri/src/platform_keyring.rs), [`src-tauri/src/vault_keychain.rs`](../src-tauri/src/vault_keychain.rs), [`src-tauri/src/external_credential_store.rs`](../src-tauri/src/external_credential_store.rs), and [`src-tauri/src/runtime_credentials.rs`](../src-tauri/src/runtime_credentials.rs)
- Focused model/UI/migration/interchange/project/sync/native regressions and exact verification record: [`src/lib/environmentSecrets.test.ts`](../src/lib/environmentSecrets.test.ts), [`src/App.environmentSecrets.test.tsx`](../src/App.environmentSecrets.test.tsx), [`src/lib/storage.test.ts`](../src/lib/storage.test.ts), [`src/lib/security.test.ts`](../src/lib/security.test.ts), [`src/lib/project.test.ts`](../src/lib/project.test.ts), [`src/lib/interchange/exporters.test.ts`](../src/lib/interchange/exporters.test.ts), [`src/lib/interchange/importers.test.ts`](../src/lib/interchange/importers.test.ts), [`src-tauri/src/platform_keyring.rs`](../src-tauri/src/platform_keyring.rs), and [`docs/QA_MILESTONE_277.md`](QA_MILESTONE_277.md)

## Milestone 278 acceptance evidence

- Pinned encrypted repository, discovery, recipient-key, and comments/presence reconciliation plus complete local/remote verification: [`docs/QA_MILESTONE_278.md`](QA_MILESTONE_278.md)
- Workspace v50 object-scoped snapshots, staging, dirty detection, commits, bounded branches/history, restore, checkout/delete, and three-way merge resolution: [`src/lib/collaboration.ts`](../src/lib/collaboration.ts), [`src/components/CollaborationRepositoryPanel.tsx`](../src/components/CollaborationRepositoryPanel.tsx), and [`src/types.ts`](../src/types.ts)
- Clean five-second remote refresh plus project/import/sync authority boundaries and device-local state preservation: [`src/components/SecurityWorkbench.tsx`](../src/components/SecurityWorkbench.tsx), [`src/lib/security.ts`](../src/lib/security.ts), [`src/lib/project.ts`](../src/lib/project.ts), and [`src/lib/storage.ts`](../src/lib/storage.ts)
- OS-protected X25519 identities, public invitations, authenticated recipient roster, HKDF-SHA256 key wrapping, fresh AES-256-GCM rotation, downgrade refusal, tamper detection, and revocation regression: [`src-tauri/src/secure_store.rs`](../src-tauri/src/secure_store.rs), [`src-tauri/src/platform_keyring.rs`](../src-tauri/src/platform_keyring.rs), and [`src-tauri/src/lib.rs`](../src-tauri/src/lib.rs)
- Focused model/UI/migration/project/sync/native regressions and exact workflow/signature evidence: [`src/lib/collaboration.test.ts`](../src/lib/collaboration.test.ts), [`src/components/CollaborationRepositoryPanel.test.tsx`](../src/components/CollaborationRepositoryPanel.test.tsx), [`src/components/SecurityWorkbench.vaultKey.test.tsx`](../src/components/SecurityWorkbench.vaultKey.test.tsx), [`src/lib/security.test.ts`](../src/lib/security.test.ts), [`src/lib/project.test.ts`](../src/lib/project.test.ts), [`src/lib/storage.test.ts`](../src/lib/storage.test.ts), and [`docs/QA_MILESTONE_278.md`](QA_MILESTONE_278.md)

## Milestone 279 acceptance evidence

- Pinned SSO, SCIM, RBAC, organization, permission, and storage-rule reconciliation plus exact verification: [`docs/QA_MILESTONE_279.md`](QA_MILESTONE_279.md)
- Workspace v51 organization/member/team/domain/invitation/grant/SSO/SCIM migration and exact permission/resource-scope rules: [`src/types.ts`](../src/types.ts), [`src/lib/storage.ts`](../src/lib/storage.ts), [`src/lib/identityGovernance.ts`](../src/lib/identityGovernance.ts), and [`src/lib/identityGovernance.test.ts`](../src/lib/identityGovernance.test.ts)
- Free organization, invitation, ownership, storage, team, resource-grant, validated-login, connector-token, server-status, and audit UI: [`src/components/IdentityGovernancePanel.tsx`](../src/components/IdentityGovernancePanel.tsx), [`src/components/SecurityWorkbench.tsx`](../src/components/SecurityWorkbench.tsx), [`src/App.tsx`](../src/App.tsx), and [`src/styles.css`](../src/styles.css)
- Native OS-protected OIDC/SCIM credentials, verified-domain HTTPS proof, OIDC discovery/PKCE/nonce/token validation, signed SAML correlation/replay validation, SCIM 2.0 Users/Groups service, automatic token renewal, and request logs: [`src-tauri/src/identity_control_plane.rs`](../src-tauri/src/identity_control_plane.rs), [`src-tauri/src/platform_keyring.rs`](../src-tauri/src/platform_keyring.rs), [`src-tauri/src/lib.rs`](../src-tauri/src/lib.rs), and [`src/lib/identityControlPlane.ts`](../src/lib/identityControlPlane.ts)
- Operator and trust-boundary documentation: [`docs/IDENTITY_GOVERNANCE.md`](IDENTITY_GOVERNANCE.md), [`docs/SECURITY_AND_SYNC.md`](SECURITY_AND_SYNC.md), and [`docs/FREE_FEATURE_POLICY.md`](FREE_FEATURE_POLICY.md)

## Milestone 280 acceptance evidence

- Pinned updater, channel, administrator/portable guard, signing, accessibility, and packaging reconciliation plus complete local/rendered verification: [`docs/QA_MILESTONE_280.md`](QA_MILESTONE_280.md)
- Signed native checks, version revalidation, verified staging, progress, restart installation, Linux notice-only behavior, and updater policy: [`src-tauri/src/desktop_update.rs`](../src-tauri/src/desktop_update.rs), [`src/lib/desktopUpdates.ts`](../src/lib/desktopUpdates.ts), and [`src/components/DesktopUpdateProvider.tsx`](../src/components/DesktopUpdateProvider.tsx)
- General Preferences update controls, semantic tab keyboard behavior, skip/main/live-region contracts, reduced motion, increased contrast, forced colors, and focused regressions: [`src/components/PreferencesWorkbench.tsx`](../src/components/PreferencesWorkbench.tsx), [`src/App.tsx`](../src/App.tsx), [`src/styles.css`](../src/styles.css), [`src/components/PreferencesWorkbench.accessibility.test.tsx`](../src/components/PreferencesWorkbench.accessibility.test.tsx), and [`src/lib/accessibilityContracts.test.ts`](../src/lib/accessibilityContracts.test.ts)
- Credential-required universal macOS/Windows/Linux trusted releases, platform verification, signed updater archives, stable/beta manifests, and unchanged unsigned provenance builds: [`.github/workflows/desktop-release.yml`](../.github/workflows/desktop-release.yml), [`scripts/create-tauri-release-config.mjs`](../scripts/create-tauri-release-config.mjs), [`scripts/updater-manifest.mjs`](../scripts/updater-manifest.mjs), and [`scripts/windows-sign.ps1`](../scripts/windows-sign.ps1)
- User/operator trust, update, and assistive-access boundaries: [`docs/DESKTOP_UPDATES_AND_ACCESSIBILITY.md`](DESKTOP_UPDATES_AND_ACCESSIBILITY.md) and [`docs/FREE_FEATURE_POLICY.md`](FREE_FEATURE_POLICY.md)

## Parity declaration rule

Brunomnia must not be described as feature-complete while any row is `Baseline`, `Early baseline`, or `Not started`. Milestone 280 leaves no such row, so the pinned clean-room behavior ledger is feature-complete within each row's documented compatibility and platform bounds. Before any later parity release, re-read the current Insomnia documentation and changelog, add newly documented capability rows, and attach reproducible evidence for every row. Commercial availability in Insomnia does not remove a capability from this ledger; Brunomnia's implementation remains governed by [the free feature policy](FREE_FEATURE_POLICY.md).
