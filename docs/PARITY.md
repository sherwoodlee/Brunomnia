...
| Mock servers | Baseline | Real native loopback mock server with routes, parameters, headers, delays, CORS, request-aware output from headers/query/path segments/raw or parsed JSON/form/multipart bodies with repeated multipart arrays and the `default` filter, bounded `assign`/`if`/`unless`/`else`/`raw` controls, all 118 currently documented Faker names, three legacy dynamic tokens, direct active-response-to-new-route creation or selected-route response overwrite, and optional AI generation from pasted prompt/spec/example material, an explicitly fetched/reviewed specification URL, or credential-redacted active-request/latest-response context. Repeated query/URL-encoded arrays, multipart metadata properties, broader Liquid operators/filters, exact FakerJS corpus/distribution identity, response-pane server selection, and hosted/self-host deployment workflows remain. | [Mock servers](https://developer.konghq.com/insomnia/mock-servers/), [Faker variables](https://developer.konghq.com/insomnia/faker-variables/) |
