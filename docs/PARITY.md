...
| Service integrations | Baseline | A pull-only Konnect adapter lists control planes and maps Gateway Services into source-backed route/path/protocol folders plus every supported HTTP/HTTPS method/path/protocol combination, WS/WSS path, and gRPC/GRPCS service-method path. Resolved-path request names, regex parameters, Host/header constraints, gRPC metadata, bounded control-plane `proxy_urls` with loopback fallback, remote-template sanitization, stable managed keys, stale managed folder/header removal, and preservation of local query/auth/body/transport/script/test/custom-header/proxy/folder work are covered. Missing-identifier, missing-service, expression-router, SNI, and L4 routes remain visible with reasons; pinned Insomnia likewise skips SNI/L4 and never pushes Gateway configuration, so those prior ledger requirements were false. Automatic all-control-plane project/workspace reconciliation, reviewed expression conversion, live-tenant fixtures, and newly documented adapters remain. | [Konnect integration](https://developer.konghq.com/insomnia/konnect-integration/) |
