...
| Mock servers | Baseline | Real native loopback mock server with routes, parameters, headers, delays, CORS, three dynamic tokens, and optional AI generation from pasted prompt/spec/example material or explicitly selected credential-redacted active-request/latest-response context. Liquid/Faker breadth, direct response-to-route conversion, URL-to-mock generation, and hosted/self-host deployment workflows remain. | [Mock servers](https://developer.konghq.com/insomnia/mock-servers/) |
