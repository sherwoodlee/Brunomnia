...
| Plugins and extension API | Baseline | Disabled-by-default local CommonJS plugins run in disposable Workers with explicit grants. Desktop HTTP, GraphQL, gRPC, and collection sends expose the documented request/body/header/parameter/auth/environment/cookie/settings context, exact response bytes/stream/duplicate headers, local store, normalized secondary requests, app dialog/prompt/clipboard/path helpers, and host-mediated data import/export with a separate private-value grant. Request, request-group, workspace, and document actions receive selected/descendant models; themes support mapped style colors plus bounded sanitized raw CSS. Linked local package metadata and explicit reload preserve the package root while disabling code and clearing grants. `--allow-plugins` additionally runs granted request/response hooks and tags for portable collection/suite HTTP/GraphQL sends inside stricter resource-limited workers without host RPC or persistent writes. Remote/npm dependency installation, external plugin-directory discovery/watch, exact context-menu placement, broad Node/plugin-ecosystem compatibility, and CLI host RPC/user-invoked actions remain. Pinned realtime connect routes bypass plugin hooks, so streaming hooks are not a parity requirement. | [Plugins](https://developer.konghq.com/insomnia/plugins/), [plugin reference](https://developer.konghq.com/insomnia/plugins/plugin-reference/), [hooks and actions](https://developer.konghq.com/insomnia/plugins/hooks-and-actions/) |
