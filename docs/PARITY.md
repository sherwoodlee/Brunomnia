...
| Secrets and external vaults | Baseline | A passphrase-derived AES-256-GCM local vault keeps decrypted values in memory, resolves `vault.*` variables, and can be exposed to desktop scripts only through a device-local off-by-default grant. Private global environments now expose the pinned String/JSON/Secret row types only while local vault authority is available: Secret inputs are masked/revealable, workspace v46 stores empty row values plus owner-bound encrypted vault entries, inherited rows resolve as `vault.<name>`, direct vault entries win collisions, conversion back to plaintext requires confirmation, and raw/shared/public/collection/folder boundaries fail closed. Debounced edits flush before project switching or locking; duplicate/delete/reset clean the matching hidden entries. Every export omits Secret rows with an explicit warning, and Insomnia `kvPairData`/`__insomnia_vault` ciphertext is warned and omitted because its account key is non-portable. Optional per-project macOS Keychain retention matches the pinned **Save encrypted vault key locally** behavior: startup/project switching can unlock through native operation-specific commands, while the renderer receives only retained/supported status, decrypted entries, and save results—never the saved key. Retention survives soft delete/restore, is removed on reset/permanent purge/Empty, and stale keys are discarded. Device-local private sub-environment trees are omitted from exports/projects/encrypted sync. Desktop tags use installed AWS, GCP, Azure, and HashiCorp official-CLI credential chains or free non-syncable macOS Keychain profiles matching the pinned AWS temporary/file/SSO, GCP service-account, HashiCorp token/AppRole/HCP Vault Secrets, and Azure OAuth record families. Guided account-free Azure browser/device-code authentication and explicit renewal use only the official Azure CLI, stream bounded login instructions, never stream token/account output, preserve profile identity, and expose no manual token editor or borrowed public client ID. Native resolution keeps secrets out of child arguments, uses bounded direct HTTPS for HCP and selected Azure profiles, validates Azure expiry/service hosts, and binds the credential plus HCP app coordinates into approval/cache identity. The explicitly granted portable CLI retains ambient official-CLI behavior and rejects device-profile arguments rather than silently changing authority. macOS Keychain also protects request/folder/MCP OAuth runtime state and reusable Git provider credentials. Silent Azure background renewal, non-macOS protected profile stores, script access to external providers, and Secret types outside private global environments remain. | [External vault integration](https://developer.konghq.com/insomnia/external-vault/), [environments](https://developer.konghq.com/insomnia/environments/) |
...
- Project-scoped macOS Keychain status/retain/forget plus saved-key unlock/save operations and stale-key refusal: [`src-tauri/src/vault_keychain.rs`](../src-tauri/src/vault_keychain.rs) and [`src-tauri/src/lib.rs`](../src-tauri/src/lib.rs)
