...
M229 closes reusable HTTP initialization, project-scoped session/protocol headers, explicit termination, and bounded server-rejected session replacement. Milestone 230 later adds persistent project/client-scoped STDIO sessions, explicit termination, retained-process cancellation, and fatal-session cleanup; Milestone 234 later closes recursive/conditional schema forms. Long-lived GET/POST SSE resumption/reconnect, reviewed per-client STDIO environment overrides, elicitation and reviewed sampling UI, notification response UI, multiple authorization-server failover, DPoP, live third-party fixtures, and OS-keychain-wrapped runtime credentials remain. MCP clients stay `Baseline`; exactly 19 parity rows remain incomplete, so Brunomnia is not feature-complete.
