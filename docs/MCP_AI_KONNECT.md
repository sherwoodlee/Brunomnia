...
The Konnect adapter accepts only HTTPS hosts equal to `api.konghq.com` or ending in `.api.konghq.com`. A selected-plane pull uses the configured protected PAT/system-token reference to:

1. list `/v2/control-planes`;
2. fetch Gateway Services for the selected control plane; and
3. fetch each Gateway Service's Routes.

Control planes use the pinned `page[size]=100&page[number]=…` contract; Services and Routes use the pinned `size=100&offset=…` contract. Repeated offsets are rejected. Redirects and cookies are disabled, requests have a 30-second deadline, `429` responses retry at most five times after their positive `Retry-After` seconds or bounded 1–30 second exponential backoff, pagination is capped at 100 pages, resource accumulation at 10,000 records, and each parsed response at 20 MB after transport buffering.

**Sync all control planes** derives the standard `us`, `eu`, `au`, `in`, and `sg` API origins from the reviewed endpoint. It creates or updates one local project per control plane without activating it, adds one root Environment when missing, and maps one Collection per Gateway Service. Route requests are fetched in batches of five Services. Control-plane project metadata retains region, cluster type, and pinned deployment type; sync reports total/created/updated/deleted/skipped counts, route reasons, failed regions, progress, and duration. A failed region cannot delete any of its existing managed projects. A successful region removes stale control-plane projects and stale service Collections. The pinned `sg` permission-gap behavior remains non-fatal.

Only the coordinator project retains the protected PAT reference. Managed projects store an empty token and disabled integration plus bounded ownership metadata. Creating them does not switch the active project; the app invalidates any stale coordinator autosave and locks project switching for the catalog mutation. Imported, shared, or manually duplicated documents have ownership metadata removed, so untrusted portable data cannot claim automatic deletion authority.
...
The integration is intentionally pull-only, matching pinned Insomnia: Brunomnia never writes Gateway configuration. Pinned SNI and L4 routes are explicit skips rather than executable requests. No live PAT is checked into the repository. `npm run test:konnect-live` is an opt-in tenant fixture gated by `BRUNOMNIA_KONNECT_LIVE=1`, `BRUNOMNIA_KONNECT_TOKEN`, and optional `BRUNOMNIA_KONNECT_REGION`; the raw token exists only in the test process's vault map.
...
- AI and coordinator Konnect configuration are device-local when opening or pulling a project and are excluded from encrypted shared-file payloads. Managed Konnect projects never receive the coordinator credential.
...
Reference behavior was reconciled against Kong's current [MCP client documentation](https://developer.konghq.com/insomnia/mcp-clients-in-insomnia/), [Konnect integration guide](https://developer.konghq.com/insomnia/konnect-integration/), and [Konnect API overview](https://developer.konghq.com/konnect-api/) on 2026-07-20. The complete official Insomnia documentation tree at Kong/developer.konghq.com commit `73995e32ed758882a290c945807225d7442b483e` contains no service-integration page beyond Konnect.
