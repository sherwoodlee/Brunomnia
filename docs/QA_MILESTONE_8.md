...
- STDIO starts a fresh direct child process for each operation. Milestone 228 adds active-operation protocol cancellation and child termination; Milestone 230 later replaces that fresh-process path with persistent project/client-scoped processes, explicit disconnect, retained-process cancellation, and fatal-session cleanup. Sampling review and elicitation forms remain open.
