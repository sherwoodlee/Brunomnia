...
- **Project files** lists Collection, Document, Mock Server, Environment, and MCP Client workspaces inside the active project. Any file can be duplicated into the current or another healthy local project with a chosen name and fresh nested resource identities, or moved to another project while preserving its identity and owned local evidence. The destination opens after either operation.
...
Move preserves authored identities. A Collection carries its suites, saved suite results, Runner reports, request activity, responses, stream sessions, and response-viewer state. A Document carries its generated collection and that collection's owned evidence. Mock routes, complete Environment descendants, and MCP connection history move with their owning file. Cookie data is copied into the destination's current project-level jar and retained in the source so moving one file cannot remove cookies still used by another file.

## Current boundary

Brunomnia exposes the five typed workspace scopes plus same/cross-project duplication and identity-preserving cross-project moves while retaining one atomic versioned document per local project. Current Insomnia stores each workspace as a separate database record and permits a project with no files. Separate per-file physical records, truly empty projects, per-file cookie/certificate isolation, permanent trash purge/retention controls, preservation of original catalog timestamps across restore, multi-version backup browsing, project discovery, and cloud/provider onboarding remain parity work.
