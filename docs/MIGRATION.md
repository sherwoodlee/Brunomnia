...
| Headless CLI | Complete baseline | Lint/generate/export plus collection/test runs using the shared OpenAPI and runner modules; stored resources accept exact names or unambiguous ID prefixes, while explicit, CI, interactive, and non-interactive collection-environment selection now follow the pinned decision boundary |
