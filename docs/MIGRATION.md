...
Compatibility bounds at this milestone were explicit: Brunomnia did not claim byte-identical Mocha reporter cosmetics. Request selection/drag ordering, response bodies in reports, `--bail`, test-name filtering, every Inso command/configuration flag, all protocol semantics, and signed container/install artifacts were open. Milestones 199–222 later close the audited user-facing command/flag surface, and Milestone 223 adds the pinned non-root multi-architecture container plus keyless-signing release path.
...
Compatibility bounds remain explicit: Insomnia v5 cannot represent standalone suites on collection-only documents, so Brunomnia emits a downgrade warning unless a linked API specification is selected. Milestone 210 later closes portable CLI per-request TLS/proxy/client-identity authority, and Milestones 211–222 close the audited pinned user-facing command/flag inventory plus bounded static configuration/global-option behavior. Executable JavaScript/TypeScript configuration, exact error/edge semantics, plugin tags, and the desktop local vault remain. Rendered Browser QA remains omitted by the standing local-browser restriction. Collection runner, Headless CLI, and Import/export stay `Baseline`; exactly 19 parity rows remain incomplete, so Brunomnia is not declared feature-complete.
