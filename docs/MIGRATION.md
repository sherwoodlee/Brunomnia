...
| MCP STDIO transport | Complete baseline | One project/client-scoped direct executable process without a shell, persistent initialization across discovery/invocation, serialized monotonic request IDs, explicit disconnect/configuration replacement, cancellation that retains the child, fatal-session cleanup, roots responses, bounded sessions/stdout/stderr/events/parameters, deadline enforcement, and explicit refusal of unreviewed server sampling/elicitation requests |
...
Compatibility bounds remain explicit: Insomnia v5 cannot represent standalone suites on collection-only documents, so Brunomnia emits a downgrade warning unless a linked API specification is selected. Milestone 210 later closes portable CLI per-request TLS/proxy/client-identity authority, Milestones 211–222 close the audited pinned user-facing command/flag inventory plus bounded static configuration/global-option behavior, Milestone 225 adds stored-plugin template tags, and Milestone 226 adds explicit-grant bounded JavaScript/TypeScript configuration. Exact error/edge semantics, arbitrary external plugin-directory discovery, desktop plugin host RPC, and the desktop local vault remain. Rendered Browser QA remains omitted by the standing local-browser restriction. Collection runner, Headless CLI, and Import/export stay `Baseline`; exactly 19 parity rows remain incomplete, so Brunomnia is not declared feature-complete.
