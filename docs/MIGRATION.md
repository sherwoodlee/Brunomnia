...
| Source emptying | Complete | Workspace v42 lets moving the final Collection, generated-Collection Document, or root Environment leave the source with zero typed files and clears stale active identities without manufacturing replacements |
...
Compatibility bounds remain explicit: local projects still use one atomic document rather than separately persisted workspace records, cookie data remains project-level and therefore copies rather than transfers, and certificates/preferences/policies remain project-level. Milestone 250 closes the former fallback-source limitation. Local projects and Collections stay `Baseline`; exactly 15 parity rows remain incomplete, so Brunomnia is not declared feature-complete. Rendered interaction and assistive-technology QA remain omitted by standing direction.
...
| Empty persistence | Complete | Workspace v42 preserves explicit zero-length Collection and Environment arrays while legacy v41 empty documents retain their repair behavior; initial, named, duplicated-empty, and final-file-move projects survive browser/native persistence without hidden typed files |
...
| Executable evidence | Complete baseline | Focused v42 migration/catalog/move/static-render regressions, strict TypeScript, full frontend/native suites, build/package, CLI, audit, and signed-image evidence are recorded in the milestone verification file |

Compatibility bounds remain explicit: Brunomnia still stores one atomic project document rather than separate physical workspace records, and cookie/certificate state remains project-level. Empty-project behavior no longer blocks either row; exactly 15 parity rows remain incomplete, so Brunomnia is not declared feature-complete. Rendered interaction and assistive-technology QA remain omitted by standing direction.
