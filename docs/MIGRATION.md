...
Compatibility bounds at this milestone remain explicit: shared-file encryption uses one team passphrase rather than per-user public-key wrapping; synchronization is pull/push; resource-specific branches/history and offline merge UI remain. Local roles are policy metadata and action checks, not strong identity authentication. Self-hosted SAML/OIDC login, SCIM provisioning, tamper-evident remote audit storage, provider SDK login flows, other provider-secret Keychain adapters, and script access to external providers remain later closure work. Milestones 239, 254, and 269 later add Keychain protection for OAuth runtime credentials, reusable Git credentials, and optional local-vault keys; Milestone 134 closes explicitly granted HTTP/GraphQL CLI external-vault tags; Milestone 278 later closes per-user wrapping, key rotation/revocation, object-scoped repository history/branches/merge, and automatic clean refresh after confirming comments and presence are not pinned product surfaces.
...
## Milestone 278 — account-free encrypted collaboration repository (complete)

| Capability | Status | Notes |
| --- | --- | --- |
| Pinned collaboration audit | Complete | Pinned source exposes encrypted commit/push, resource branches, history/rollback, merge, and synchronization; no mounted product comments or presence surface exists, correcting those prior ledger requirements |
| Object-scoped repository | Complete | Workspace v50 snapshots shareable Collections, root Environment branches, API Designs, Mock Servers, and MCP Clients with staging, dirty detection, 200 commits and 50 branches per resource, restore-as-commit, checkout/delete, and bounded authority stripping |
| Offline merge and refresh | Complete | Three-way JSON merge applies disjoint edits automatically, requires explicit mine/theirs/manual JSON choices for conflicts, and five-second polling pulls newer clean revisions while staged or dirty resources block automatic replacement |
| Per-user encryption | Complete | OS-protected X25519 device identities publish public invite codes; every push rotates a fresh AES-256-GCM content key wrapped independently with HKDF-SHA256 for up to 100 authenticated recipients, removal takes effect on the next revision, and recipient files cannot silently downgrade |
| Publication boundaries | Complete | Split-project writes and imports cannot carry collaboration authority; encrypted payloads omit private environments, OAuth runtime data, responses, cookies, reports, paths, plugins, preferences, vault contents, certificates, and recipient private keys while matching local state survives pull |
| Verification | Complete | Collaboration model/UI, migration, project, sync, authenticated-recipient tamper/revocation, TypeScript, frontend/native, build, packaging, and remote workflow evidence is recorded in [Milestone 278 verification](QA_MILESTONE_278.md) |
