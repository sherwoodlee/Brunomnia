...
M231 closes reviewed project-scoped STDIO environment rows, template/vault resolution, plaintext-sensitive-value refusal, ambient-environment isolation, bounded native dispatch, workspace v38 persistence, and environment-triggered persistent-process replacement. Milestones 232–234 later add bounded login-shell `PATH` discovery, first-class Insomnia v4/v5 MCP-resource import/export, and recursive/conditional schema forms. Long-lived GET/POST SSE resumption/reconnect, elicitation and reviewed sampling UI, notification/server-request response UI, multiple authorization-server failover, DPoP, live third-party fixtures, and OS-keychain-wrapped runtime credentials also remain. MCP clients and Import/export stay `Baseline`; exactly 19 parity rows remain incomplete, so Brunomnia is not feature-complete.
