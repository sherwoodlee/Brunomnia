...
const mcpSessionConfigurationFields = new Set<keyof McpClient>(['transport', 'url', 'headers', 'authType', 'token', 'username', 'password', 'oauthAuthorizationUrl', 'oauthAccessTokenUrl', 'oauthClientId', 'oauthClientSecret', 'oauthScope', 'oauthState', 'oauthRegisteredClientId', 'oauthRegisteredClientSecret', 'oauthRegisteredClientIdIssuedAt', 'oauthRegisteredClientSecretExpiresAt', 'oauthRegisteredTokenEndpointAuthMethod']);
...
export function IntegrationWorkbench({ workspace, workspaceId, environment, requestContext: baseRequestContext, onChangeWorkspace }: IntegrationWorkbenchProps) {
  ...
  const connected = Boolean(active?.transport === 'http' && hasMcpClientSession(active, workspaceId));
  ...
  const invalidateActiveSession = () => {
    if (!active || active.transport !== 'http') return;
  ...
  const disconnect = () => active && canEdit && run('Disconnecting MCP client', async () => {
    ...
    refreshSessionState();
    setMessage(output.terminated ? 'Disconnected and terminated the remote MCP HTTP session.' : 'Disconnected the local MCP HTTP session.');
  });
