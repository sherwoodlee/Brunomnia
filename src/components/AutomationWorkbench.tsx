...
const runnerReportDuration = (report: RunnerReport) => {
...
function RunnerWorkbench({ workspace, workspaceId, activeEnvironment, vault, onChangeWorkspace, templatePrompt, runnerTarget, onRunnerStart, runnerDraft, runnerDraftKey, onRunnerDraftChange }: AutomationWorkbenchProps) {
  ...
  const [resultStatusFilter, setResultStatusFilter] = useState<RunnerResultFilter>('all');
  ...
  const visibleResults = selectedReport ? selectedReport.results : results.length ? results : displayedReport?.results ?? [];
  const visibleLiveItems = selectedReport?.liveItems?.length ? selectedReport.liveItems : !selectedReport && liveItems.length ? liveItems : displayedReport?.liveItems?.length ? displayedReport.liveItems : visibleResults.map((result): RunnerLiveItem => ({
  ...
  const filteredLiveItems = filterRunnerLiveItems(visibleLiveItems, visibleResults, resultStatusFilter, resultNameFilter);
  ...
  const selectedResult = selectedResultCandidate && filteredLiveItems.some((item) => resultForItem(item)?.id === selectedResultCandidate.id) ? selectedResultCandidate : undefined;
  ...
  const visibleKeepLog = running ? keepLog : displayedReport?.keepLog ?? keepLog;
  return (
    <section className="automation-workbench runner-workbench">
      ...
      <div className="runner-grid">
        ...
        <div className="runner-results">
          ...
          {resultPane === 'results' ? <><div className="runner-table"><div className="runner-result-filters"><nav aria-label="Filter Runner results">{(['all', 'passed', 'failed', 'skipped'] as RunnerResultFilter[]).map((filter) => <button aria-pressed={resultStatusFilter === filter} key={filter} onClick={() => setResultStatusFilter(filter)} type="button">{filter[0].toUpperCase() + filter.slice(1)}</button>)}</nav><input aria-label="Filter Runner results by name" onChange={(event) => setResultNameFilter(event.target.value)} placeholder="Filter test results with name" type="search" value={resultNameFilter} /><span>{filteredLiveItems.length}/{visibleLiveItems.length}</span></div><div className="runner-table-head"><span>Request</span><span>Iteration</span><span>Attempt</span><span>Status</span><span>Response</span><span>Tests / error</span><span>Action</span></div>{filteredLiveItems.map((item) => {
          ...
          })}{!visibleLiveItems.length ? <div className="empty-state compact"><Icon name="history" size={28} /><strong>No runner results</strong><span>Start the selected collection to build a local report.</span></div> : !filteredLiveItems.length ? <div className="empty-state compact"><Icon name="search" size={28} /><strong>No matching runner results</strong><span>Change the status or name filter.</span></div> : null}</div>
