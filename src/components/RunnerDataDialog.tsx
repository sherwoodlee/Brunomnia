...
import { RUNNER_DATA_FILE_BYTES, parseRunnerDataFile, type RunnerDataFilePreview } from '../lib/runner';
import { Icon } from './Icon';

type RunnerDataDialogProps = {
  data: string;
  fileName: string;
  onApply: (data: string, fileName: string, rowCount: number) => void;
  onClear: () => void;
  onClose: () => void;
};
...
export function RunnerDataDialog({ data, fileName: initialFileName, onApply, onClear, onClose }: RunnerDataDialogProps) {
  ...
  return (
    <div className="runner-data-overlay" onMouseDown={onClose} role="presentation">
      <section aria-labelledby="runner-data-title" aria-modal="true" className="runner-data-dialog" onMouseDown={(event) => event.stopPropagation()} role="dialog">
        ...
        {fileName ? <p className="runner-data-source"><strong>{fileName}</strong><span>{preview?.rows.length ?? 0} iterations · {preview?.headers.length ?? 0} variables</span></p> : null}
