...
describe('local project catalog', () => {
  ...
  it('creates initial and named local projects with no typed files', async () => {
    const initial = await loadWorkspaceCatalog();
    expect(initial.workspace).toMatchObject({ name: 'Local Workspace', version: 46, activeRequestId: '', activeEnvironmentId: '', collections: [], environments: [], fileState: {} });
  ...
  it('preserves explicit v42 empty projects while repairing legacy empty documents', () => {
    ...
    expect(migrateWorkspace(empty)).toMatchObject({ version: 46, activeRequestId: '', activeEnvironmentId: '', collections: [], environments: [], fileState: {} });
