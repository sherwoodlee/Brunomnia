...
describe('local project catalog', () => {
  ...
  it('normalizes bounded collaboration branches and strips imported repository authority', () => {
    const workspace = cloneSeedWorkspace() as unknown as Record<string, unknown>;
    workspace.collaboration = {
      mode: 'encrypted-file', path: '/shared/project.enc.json', actor: 'Avery', revision: 4, autoSync: true,
      stagedResourceKeys: ['collection:orders', 'invalid', 'collection:orders'],
      repository: {
        version: 99,
        activeBranches: { 'collection:orders': 'feature/orders', invalid: 'main' },
        branches: [
          { resourceKey: 'collection:orders', name: 'feature/orders', headCommitId: 'commit-one', createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-02T00:00:00Z' },
          { resourceKey: 'collection:orders', name: '../unsafe', headCommitId: '', createdAt: '', updatedAt: '' },
        ],
        commits: [
          { id: 'commit-one', resourceKey: 'collection:orders', branch: 'feature/orders', parentId: '', actor: 'Avery', message: 'Safe', createdAt: '2026-01-02T00:00:00Z', snapshot: { collection: { id: 'orders' } } },
          { id: 'commit-two', resourceKey: 'invalid', branch: 'main', parentId: '', actor: '', message: '', createdAt: '', snapshot: {} },
        ],
      },
    };

    const migrated = migrateWorkspace(workspace);
    expect(migrated.collaboration).toMatchObject({ mode: 'encrypted-file', revision: 4, autoSync: true, stagedResourceKeys: ['collection:orders'] });
    expect(migrated.collaboration.repository).toEqual({
      version: 1,
      activeBranches: { 'collection:orders': 'feature/orders' },
      branches: [{ resourceKey: 'collection:orders', name: 'feature/orders', headCommitId: 'commit-one', createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-02T00:00:00Z' }],
      commits: [{ id: 'commit-one', resourceKey: 'collection:orders', branch: 'feature/orders', parentId: '', actor: 'Avery', message: 'Safe', createdAt: '2026-01-02T00:00:00Z', snapshot: { collection: { id: 'orders' } } }],
    });
    expect(secureImportedWorkspace(workspace).collaboration).toEqual(cloneSeedWorkspace().collaboration);
  });
  ...
  it('retains Secret metadata only in private collection sub-environments', () => {
    const workspace = cloneSeedWorkspace() as unknown as Record<string, unknown>;
    const collection = (workspace.collections as Array<Record<string, unknown>>)[0];
    collection.subEnvironments = [
      { id: 'shared', name: 'Shared', variables: [{ id: 'shared-secret', name: 'token', value: 'untrusted', valueType: 'secret', enabled: true }] },
      { id: 'private', name: 'Private', private: true, environmentEditorMode: 'raw', variables: [{ id: 'private-secret', name: 'token', value: 'untrusted', valueType: 'secret', enabled: true }] },
    ];
    collection.activeSubEnvironmentId = 'private';
    const migrated = migrateWorkspace(workspace);
    expect(migrated.collections[0].subEnvironments?.[0].variables).toEqual([]);
    expect(migrated.collections[0].subEnvironments?.[1]).toMatchObject({ private: true, environmentEditorMode: 'table', variables: [{ id: 'private-secret', name: 'token', value: '', valueType: 'secret', enabled: true }] });
    expect(migrated.collections[0].activeSubEnvironmentId).toBe('private');
  });
