...
describe('workspace migrations', () => {
  it('upgrades v1 requests with protocol defaults without changing request identity', () => {
    ...
    expect(migrated.preferences).toMatchObject({ theme: 'system', preferredHttpVersion: 'default', maxRedirects: 10, requestTimeoutMs: 30_000, autoFetchGraphqlSchema: true });
  ...
  it('disables imported plugin code and clears inherited authority', () => {
    ...
    exported.preferences = { ...exported.preferences, theme: 'light', preferredHttpVersion: 'http2-prior-knowledge', maxRedirects: -1, requestTimeoutMs: 123_000, allowScriptRequests: true, allowScriptFileAccess: true, enableVaultInScripts: true };
    ...
    expect(imported.preferences).toMatchObject({ theme: 'system', preferredHttpVersion: 'default', maxRedirects: 10, requestTimeoutMs: 30_000, allowScriptRequests: false, allowScriptFileAccess: false, enableVaultInScripts: false });
  });

  it('normalizes preference bounds and shortcut values', () => {
    const workspace = cloneSeedWorkspace() as unknown as Record<string, unknown>;
    workspace.preferences = { theme: 'unknown', density: 'compact', fontSize: 99, preferredHttpVersion: 'http3', maxRedirects: -99.8, requestTimeoutMs: 1, scriptTimeoutMs: 999_999, allowScriptRequests: 'yes', allowScriptFileAccess: 'yes', enableVaultInScripts: 1, shortcuts: { palette: ' mod + shift + p ', send: 42 } };
    const migrated = migrateWorkspace(workspace);
    expect(migrated.preferences).toMatchObject({ theme: 'system', density: 'compact', fontSize: 20, preferredHttpVersion: 'default', maxRedirects: -1, requestTimeoutMs: 1_000, scriptTimeoutMs: 60_000, allowScriptRequests: false, allowScriptFileAccess: false, enableVaultInScripts: false });
  ...
  it('preserves supported device-local HTTP preferences', () => {
    ...
    expect(migrateWorkspace(workspace).preferences).toMatchObject({ preferredHttpVersion: 'http2-prior-knowledge', maxRedirects: -1 });
  });
