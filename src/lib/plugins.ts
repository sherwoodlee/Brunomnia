...
${source}
    ...
    const ${responseApi} = response => ({
      ...
      getBytesRead: () => response.sizeBytes, getTime: () => response.durationMs, getBody: () => SafeBuffer.from(response.body),
      setBody: body => { requirePermission('response:write'); response.body = body instanceof Uint8Array ? new TextDecoder().decode(body) : String(body); response.sizeBytes = new TextEncoder().encode(response.body).length; },
