export type DocumentTabType = 'request' | 'folder' | 'runner' | 'environment' | 'document';
...
export const parseRequestTabState = (value: string | null): RequestTabState => {
  ...
  try {
    ...
    const tabs = (Array.isArray(parsed.tabs) ? parsed.tabs : []).flatMap((value): RequestDocumentTab[] => {
      ...
      const type: DocumentTabType = tab.type === 'folder' || tab.type === 'runner' || tab.type === 'environment' || tab.type === 'document' ? tab.type : 'request';
