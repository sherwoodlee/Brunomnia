...
import { applyDefaultAcceptEncodingHeader, applyDefaultAcceptHeader, calculatedRequestHeaders, DEFAULT_ACCEPT_ENCODING_HEADER, DEFAULT_ACCEPT_HEADER } from './calculatedHeaders';
...
describe('calculated request headers', () => {
  ...
  it('adds native compression negotiation only without an enabled authored row', () => {
    const headers: KeyValue[] = [];
    expect(applyDefaultAcceptEncodingHeader(headers)).toEqual([{ id: 'default-accept-encoding', name: 'Accept-Encoding', value: DEFAULT_ACCEPT_ENCODING_HEADER, enabled: true }]);
    const authored = [{ id: 'encoding', name: 'accept-encoding', value: 'identity', enabled: true }];
    expect(applyDefaultAcceptEncodingHeader(authored)).toBe(authored);
  });
  ...
  it('describes regular-editor HTTP rows and the request-level User-Agent toggle', () => {
    ...
    expect(calculatedRequestHeaders(request)).toEqual([
      ...
      expect.objectContaining({ name: 'Accept-Encoding', value: DEFAULT_ACCEPT_ENCODING_HEADER, canDisable: false }),
    ...
    expect(calculatedRequestHeaders(request).map((header) => header.name)).toEqual(['Accept', 'Accept-Encoding', 'Host']);
  });
