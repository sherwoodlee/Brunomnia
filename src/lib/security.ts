...
const protectedReference = (value: string) => value.includes('{{ vault.') || value.includes('{% external');
const sensitiveVariable = /(secret|password|token|api.?key|private.?key|client.?secret)/i;
const sensitiveRequestKey = /(authorization|api[-_]?key|token|secret|password)/i;
...
export const plaintextSecretCandidates = (workspace: Workspace): string[] => {
  const candidates = workspace.environments.flatMap((environment) => environment.variables
    .filter((variable) => variable.value && sensitiveVariable.test(variable.name) && !protectedReference(variable.value))
  ...
  workspace.collections.forEach((collection) => collection.requests.forEach((request) => {
    sensitiveAuthFields.forEach((field) => {
      ...
      if (typeof value === 'string' && value && !protectedReference(value)) candidates.push(`${collection.name} / ${request.name}: auth.${field}`);
    });
    request.headers.filter((header) => header.value && sensitiveRequestKey.test(header.name) && !protectedReference(header.value))
      .forEach((header) => candidates.push(`${collection.name} / ${request.name}: header ${header.name}`));
    request.params.filter((parameter) => parameter.value && sensitiveRequestKey.test(parameter.name) && !protectedReference(parameter.value))
      .forEach((parameter) => candidates.push(`${collection.name} / ${request.name}: query ${parameter.name}`));
    if (embeddedUrlCredential.test(request.url) && !protectedReference(request.url)) candidates.push(`${collection.name} / ${request.name}: URL credentials`);
    if (request.transport.clientKeyPem && !protectedReference(request.transport.clientKeyPem)) candidates.push(`${collection.name} / ${request.name}: client private key`);
  }));
  return candidates;
};
