...
export const fetchGraphqlSchema = async (request: ApiRequest, environment: Environment | undefined, context: SendRequestContext) => {
  ...
  schemaRequest.tests = '';
  schemaRequest.transport = { ...schemaRequest.transport, followRedirects: false, timeoutMs: Math.min(120_000, Math.max(1_000, schemaRequest.transport.timeoutMs)), storeCookies: false };
