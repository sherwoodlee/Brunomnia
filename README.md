...
- Multiple account-free local projects with create/switch/rename/duplicate-any/reorder/delete/restore workflows, a five-scope typed project-file inventory, same/cross-project file duplication with fresh nested identities, identity-preserving cross-project moves with owned local evidence, one bounded manifest plus authoritative per-file physical records, encrypted vaults, aggregate-document migration, rotating backups, catalog reconstruction, recently deleted management, and individual-record corruption recovery
...
- Distinct global-base/selected-global and collection-base/selected-collection environments, persistent sub-environment drag and Option/Alt-keyboard ordering, collision-safe duplication, device-local private global sub-environments with masked encrypted Secret rows under `vault.*`, folder/iteration/request-local variables, dynamic aliases, and template tags for UUIDs, time, Faker values, encoding, hashing, JSONPath, cookies, prompts, requests, and chained responses
...
- Local CommonJS plugins with disabled-by-default pasted, linked-package, or bounded npm/custom-registry installation; same-origin metadata redirects, confined HTTPS/custom-origin tarballs, per-package SHA-1 verification, identity-aware same-package updates with unchanged-authority retention and changed-code reset, reviewable multi-file JavaScript/JSON/CJS modules plus bounded local/remote pure-CommonJS production graphs, baseline `buffer`/`path`/`crypto`, explicitly granted `events`, pinned `uuid` 11.1.1 and `ajv` 8.18.0, or reviewed dependencies, explicit source reload, request/response hooks, template tags, request/folder/workspace/document actions with pinned request/folder/design menu placement, style-aware themes, plugin-local storage, the documented request/response/context-data helpers, and the pinned sandbox-demo globals, frozen process metadata, marker, OS utility, and relative-module behavior
...
- A passphrase-derived AES-256-GCM local vault whose direct and private global/collection-environment Secret values resolve through `{{ vault.name }}`, with masked/reveal editing, debounced encrypted persistence, optional per-project operating-system credential retention, and account-free automatic unlock
- AWS Secrets Manager, GCP Secret Manager, Azure Key Vault, and HashiCorp Vault template adapters through user-authenticated official CLIs or free non-syncable operating-system credential profiles for AWS temporary/file/SSO, GCP service accounts, HashiCorp token/AppRole/HCP Vault Secrets, and Azure OAuth tokens, with guided official Azure CLI browser/device-code authentication and renewal, an in-memory cache, and an exact credential-bound allowlist
- End-to-end encrypted shared-file revisions with passphrase compatibility or OS-protected X25519 per-device recipients, public invitations, fresh key rotation/revocation, optimistic conflict checks, explicit force, five-second clean auto-pull, device-local filtering, and self-hosted filesystem/WebDAV compatibility
- Object-scoped collaboration staging, commits, bounded history, rollback, branches, checkout/delete, and three-way merge with explicit mine/theirs/manual conflict resolution for Collections, Environments, API Designs, Mock Servers, and MCP Clients
- Free self-hosted organizations with exact owner/admin/editor/viewer permissions, 30-day invitations, teams, ownership transfer, storage controls, verified domains, OIDC authorization-code/PKCE login, signed SAML 2.0 login, collection/API-design RBAC, a bearer-authenticated SCIM 2.0 Users/Groups connector with one-way Okta/Azure provisioning and request logs, OS-protected secrets/verifiers, and bounded governance audit
...
Requirements: Node.js 20.19+ (or 22.12+), Rust 1.85+, CMake, and Clang. The native build compiles its pinned llama.cpp runtime; no model is downloaded or bundled.
...
The pinned Inso command/flag and execution contract is source-audited as complete. Brunomnia intentionally adds deterministic CI fallbacks, explicit trust grants, stored-plugin request/response hooks and template tags, bounded local-reference lint, retries, and JSON/JUnit output; pinned Inso itself has no user plugin-directory loader, plugin host RPC, or desktop local-vault API.
