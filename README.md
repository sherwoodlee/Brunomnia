...
node bin/brunomnia.cjs run collection workspace.brunomnia.json collection-id --item request-third --item request-first --requestNamePattern '^(Third|First)$' --env-var region=staging --delay-request 100 --iteration-data iterations.csv --iteration-count 2 --bail
