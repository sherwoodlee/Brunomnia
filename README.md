...
- Native loopback mock servers with route parameters, delays, headers, CORS, request-aware header/query/path/JSON/form/multipart output templates, repeated multipart fields, bounded `assign`/`if`/`unless`/`raw` controls, all 118 currently documented Faker outputs, dynamic response tokens, and direct latest-response route creation/overwrite
