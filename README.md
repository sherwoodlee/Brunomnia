...
- Persistent standalone test suites with ordered suite/test CRUD, selected HTTP/GraphQL requests, JavaScript/Chai editing, bounded run-one/run-all execution, default or ID-targeted `insomnia.send()`, inherited environments, OAuth/plugins/cookies/response chaining, and saved local run results
