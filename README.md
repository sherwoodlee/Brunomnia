...
- A searchable 100-send activity log plus chronologically grouped per-request response history with finite/unlimited retention, active-environment filtering, rich URL/method/status/time/size evidence, delete/clear actions, historical request-version restoration, persistent Visual/Source/Raw preview modes with safe HTML, bounded CSV tables, and selectable multipart sections, JSONPath/XPath body filters, 5/100 MiB preview safety gates, byte-exact decoded-body/raw and prettified JSON downloads, selected-response HTTP debug/HAR exports, body/header inspection, and persisted size-bounded outgoing/response timeline evidence
