...
- Pull-only Konnect control-plane discovery and Gateway Service mapping across HTTP/HTTPS, WebSocket, and gRPC route combinations with regex path parameters, remote-template sanitization, local-work preservation, safe control-plane proxy defaults with loopback fallback, and explicit expression/SNI/L4 skips
