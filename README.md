...
node bin/brunomnia.cjs run collection collection-id --workingDir workspace.brunomnia.json --item request-third --item request-first --requestNamePattern '^(Third|First)$' --requestTimeout 30000 --env-var region=staging --delay-request 100 --iteration-data iterations.csv --iteration-count 2 --bail
