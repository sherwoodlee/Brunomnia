...
- Native loopback mock servers with route parameters, delays, headers, CORS, request-aware header/query/path/JSON/form output templates, bounded `assign`/`if`/`unless`/`raw` controls, dynamic response tokens, and direct latest-response route creation/overwrite
