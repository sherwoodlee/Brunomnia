...
- Selectable drag/keyboard-ordered collection runs with JSON/CSV iteration data, retries, bail/cancellation, cookie/response chaining, bounded GraphQL subscription/WebSocket/Socket.IO/SSE samples, redacted request metadata, size-limited response inspection, saved reports, downloadable JSON/JUnit evidence, and a shell-safe Run via CLI preview of the active plan
...
- A headless CLI for OpenAPI lint/generation/export, JSON or split-YAML project collection execution with repeated selected-request and request-delay controls, and standalone test-suite execution by suite or linked API-spec identity with regex test-name filtering, the documented Inso reporter names, and JSON/JUnit artifacts
...
node bin/brunomnia.cjs run collection workspace.brunomnia.json collection-id --request request-third --request request-first --delay-request 100 --data iterations.csv --bail
node bin/brunomnia.cjs run test examples/cli-workspace.json "CLI Health" --allow-scripts
node bin/brunomnia.cjs run test examples/cli-workspace.json cli-api-design --allow-scripts --bail --reporter junit --output report.xml
npm run test:cli-runner-smoke
npm run test:cli-template-smoke
```

The checked-in [CLI workspace fixture](examples/cli-workspace.json) is self-contained and does not make an internet request; `run test` accepts its suite name/ID or linked API-spec name/ID. `run collection` accepts a Brunomnia JSON file or connected split-YAML project directory, repeated `--request`/`-i` selectors in execution order, `--delay-request`, and the desktop Runner's remaining iteration/data/bail controls. CLI JavaScript is disabled unless `--allow-scripts` is present. Trusted scripts can make arbitrary secondary HTTP requests only when `--allow-script-requests` is also present, while a suite's saved-request `insomnia.send()` stays confined to its owning collection. Script attachments require `--allow-script-files`; built-in File tags require `--allow-template-files` or the broader script-file grant; approved external-vault tags require `--allow-external-vaults`. Imported workspace preferences cannot enable these authorities. The localhost-only smokes prove Runner command continuity plus template denial/grant behavior, saved-request execution, OS/hash/custom-time rendering, dependent responses, and cookie continuity without external network access. Reporters are `dot`, `list`, `min`, `progress`, `spec`, `tap`, `json`, and `junit`; use `--output` to write the artifact instead of stdout. See [runner reports and CI](docs/RUNNER_REPORTS.md).
