...
- A passphrase-derived AES-256-GCM local vault whose direct and private-environment Secret values resolve through `{{ vault.name }}`, with masked/reveal editing, debounced encrypted persistence, optional per-project macOS Keychain retention, and account-free automatic unlock
- AWS Secrets Manager, GCP Secret Manager, Azure Key Vault, and HashiCorp Vault template adapters through user-authenticated official CLIs or free non-syncable macOS Keychain profiles for AWS temporary/file/SSO, GCP service accounts, HashiCorp token/AppRole/HCP Vault Secrets, and Azure OAuth tokens, with guided official Azure CLI browser/device-code authentication and renewal, an in-memory cache, and an exact credential-bound allowlist
