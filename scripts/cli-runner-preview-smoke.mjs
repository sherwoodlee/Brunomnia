...
const testHelp = await run(['run', 'test', '--help'], baseEnvironment);
assert.match(testHelp, /^Usage: brunomnia run test \[identifier\] \[options\]/);
assert.match(testHelp, /--keepFile/);
...
assert.match(collectionHelp, /--includeFullData <redact\|plaintext>/);
const lintHelp = await run(['lint', 'spec', '-h'], baseEnvironment);
assert.match(lintHelp, /^Usage: brunomnia lint spec \[identifier\] \[options\]/);
assert.match(lintHelp, /--ruleset <path>/);
const arrivals = [];
